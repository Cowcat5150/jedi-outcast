...
void GLimp_Init(void)
{
	...
	if ( !R_GetModeInfo( &glConfig.vidWidth, &glConfig.vidHeight, r_mode->integer ) )
	{
			...
			if (win)
			{
				...
				if (__tglContext)
				{
					...
					if (r)
					{
						...
						if (pointermem)
						{
							SetPointer(win, pointermem, 16, 16, 0, 0);
							...
							glConfig.colorBits = GetBitMapAttr(win->WScreen->RastPort.BitMap, BMA_DEPTH);
